...
if(!is.null(opt$anno2)){ha_col <- read.table(opt$anno2,sep="\t",row.names=1,head=T)}else{ha_col<-NULL}
if(opt$col_names_side=="top"){
    bottom_annotation=HeatmapAnnotation(ha_col)
    top_annotation=NULL
}else if(opt$col_names_side=="bottom"){
    top_annotation=HeatmapAnnotation(ha_col)
